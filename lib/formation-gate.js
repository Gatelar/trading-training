...
(function () {
    ...
    // Réévalue à chaque changement de session : une connexion, un rafraîchissement
    // de jeton ou une déconnexion doivent rouvrir ou refermer l'accès sans que
    // l'utilisateur ait à recharger la page.
    if (supabaseClient.auth.onAuthStateChange) {
        supabaseClient.auth.onAuthStateChange(function (evenement) {
            if (evenement === 'SIGNED_IN' || evenement === 'SIGNED_OUT' ||
                evenement === 'TOKEN_REFRESHED' || evenement === 'INITIAL_SESSION') {
                evaluer();
            }
        });
    }
    ...
    supabaseClient.auth.getSession().then(async function (res) {
        ...
        if (!session) { ETAT = 'anonyme'; motif = null; diffuser(); return; }
        ...
        // Une erreur de lecture n'est pas la même chose qu'une absence
        // d'abonnement, et se diagnostique très mal sans la voir.
        if (r.error) {
            window.ttFormationErreur = r.error.message;
            console.warn('[formation] lecture de subscriptions refusée :', r.error.message);
        }
        diffuser();
    }).catch(function (e) {
        // Une erreur réseau ne doit pas donner l'accès par accident.
        ETAT = 'anonyme';
        window.ttFormationErreur = e && e.message;
        diffuser();
    });
    }
